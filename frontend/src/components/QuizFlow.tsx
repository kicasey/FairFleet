...
function pickMatches(answers: QuizAnswer): Destination[] {
  return [...destinations]
...
export default function QuizFlow({ onComplete, onClose, inline }: Readonly<QuizFlowProps>) {
  ...
  // Bot sends the next question after a short "typing" delay.
  useEffect(() => {
  ...
  // Auto-scroll on message/typing changes.
  useEffect(() => {
  ...
  async function finalizeWithAi(finalAnswers: QuizAnswer, note: string | undefined) {
    const matches = pickMatches(finalAnswers);
    setPhase('generating');
    setIsTyping(true);
    appendMessage({
      id: 'gen-status',
      role: 'bot',
      text: "Thinking through your matches…",
    });
  ...
  function handleSelect(option: QuizOption) {
    ...
    setTimeout(() => {
      ...
      if (isLast) {
        // Ask for an optional freeform note.
        setTimeout(() => {
                        ...
                        <div className="flex items-end justify-between mt-2">
                          <div>
                            ...
                            <p className="text-[10px] font-body text-muted mt-1">
                              {dest.flightTime} flight
                            </p>
                          </div>
                          <button className="rounded-full bg-brand-blue px-3 py-1 text-[11px] font-body font-semibold text-white hover:bg-brand-dark-blue transition-colors flex items-center gap-1">
